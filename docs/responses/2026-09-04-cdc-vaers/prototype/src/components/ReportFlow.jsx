...
import { useConfig, useT, loadAnswers, persistAnswers, clearAnswers } from "../engine/store.jsx";
...
export function ReportFlow() {
  const { schema, suggestions: suggestionRules } = useConfig();
  const { t: ui } = useT();
  ...
  const steps = useMemo(
    () => [...sections, { id: "review", title: ui("reviewStep"), review: true }],
    [sections, ui]
  );
      ...
      <div className="container">
        <div className="page" style={{ maxWidth: "46rem" }}>
          <div className="success-panel" role="status">
            ...
            <p>
              {ui("successText")}
              {submitted.reportMeta.elapsedMinutes && ui("completedIn", submitted.reportMeta.elapsedMinutes)}{" "}
              {ui("nothingStored")}
            </p>
          </div>
          ...
          <p className="report-sub">{ui("recordSub")}</p>
        ...
        {showSurvey && (
          <SurveyModal
            title={ui("postSurveyTitle")}
            prompt={ui("postSurveyPrompt")}
      ...
      <div className="report-shell">
        <main className="report-main" id="main">
          ...
          <p className="report-sub">
            {ui("reportSubA")} <span className="req-flag">*</span> {ui("reportSubB")}
          </p>

          <div
            className="mode-toggle"
            role="group"
            aria-label={ui("styleGroup")}
          >
            <button
              type="button"
          ...
          {suppressedCount > 0 && (
            <p className="branch-note">
              <strong>{ui("branchRemoved", suppressedCount)}</strong> {ui("branchWhy")}
            </p>
          )}
          ...
          {Object.keys(errors).length > 0 && (
            ...
            >
              <strong>{ui("errorsNeedAttention", Object.keys(errors).length)}</strong>
            </div>
          )}
            ...
            <>
              ...
              {suggestions.length > 0 && (
                <div className="suggest-card">
                  <h3>
                    {ui("suggestTitle")}{" "}
                    <span className="sim-tag">{ui("autoGenerated")}</span>
                  </h3>
                  <p
                    style={{
                      fontSize: "var(--fs-xs)",
